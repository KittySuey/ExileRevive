...
FAR_Player_Actions =
{
//systemChat "Adding actions...";
_newPlayerObject = _this select 0;
    if (isNull _newPlayerObject) then
    {
    _newPlayerObject = player;
    };

	if (alive _newPlayerObject && _newPlayerObject isKindOf "Man") then
	{
	//systemChat "Executing addActions...";
		...
		_newPlayerObject addAction ["<t color=""#C90000"">" + "Revive" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_revive"], 10, true, true, "", "call FAR_Check_Revive"];
		_newPlayerObject addAction ["<t color=""#C90000"">" + "Stabilize" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_stabilize"], 10, true, true, "", "call FAR_Check_Stabilize"];
		_newPlayerObject addAction ["<t color=""#C90000"">" + "Suicide" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_suicide"], 9, false, true, "", "[_newPlayerObject] call FAR_Check_Suicide"];
		_newPlayerObject addAction ["<t color=""#C90000"">" + "Drag" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_drag"], 9, false, true, "", "call FAR_Check_Dragging"];
		//_newPlayerObject addAction ["<t color=""#C90000"">" + "EXECUTE" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_finish"], 9, false, true, "", "call FAR_Check_Finish"];
	};
//systemChat "Complete.";
};
...
FAR_Player_Unconscious =
{
	...
	if (isPlayer _unit) then 
	{
		...
		while { !isNull _unit && alive _unit && _unit getVariable "FAR_isUnconscious" == 1 && _unit getVariable ["FAR_isStabilized", 0] == 0 && (FAR_BleedOut <= 0 || time < _bleedOut) } do
		{
		...
		else
		{
			// Player got revived
			//systemChat "Revived!";
			//systemChat format["FAR_BleedOut: %1, time: %2, _bleedOut: %3, FAR_isStabilized: %4", FAR_BleedOut, time, _bleedOut, _unit getVariable ["FAR_isStabilized",0]];
			...
			_unit playMove "";
			//_unit setVariable ["ExileIsDead",false,true];
		};
	}
	else
	{
...
FAR_HandleRevive =
{
	...
	if (alive _target) then
	{
		...
		sleep 6;
		systemChat "Revive Complete!";
        _target setVariable ["FAR_isUnconscious", 0, true];
        _target setVariable ["FAR_isDragged", 0, true];
       /*
        _Uncon = getVariable ["FAR_isUnconscious", 0];


                    for [{_i=0}, {_i < 10}, {_i=_i+1}] do
                    {
                    _Uncon = player getVariable ["FAR_isUnconscious", 0];
                    sleep 1;
                    _iTemp = _i + 1;
                    _iTemp = _iTemp * 10;
                        if (alive Player && _Uncon == 0) then
                        {
                        systemChat format["Reviving is %1 percent complete.",_iTemp];
                        }
                        else
                        {
                        _i = 99; //break loop
...
FAR_Check_Revive = 
{
	...
	{
	//systemChat "Aborting revive, first condition";
		_return
	};

    /*
    _isPlayerUnconscioos = player getVariable ["FAR_isUnconsciouos", 0];
        if (_isPlayerUnconscioos == 0) exitWith
        {
       // systemChat "Aborting the abort for second condition";
          //  _return
        };
        */
   // systemChat "player conditions met, continue revive check";
	_isTargetUnconscious = _target getVariable ["FAR_isUnconscious", 0];
	_isDragged = _target getVariable ["FAR_isDragged", 0];
	//systemChat "starting target revive check";
	// Make sure target is unconscious and player is a medic 
	if (_isTargetUnconscious == 1 && _isDragged == 0) then
	{
	//systemChat "Target is eligible to be revived";
		_return = true;

		// [ReviveMode] Check if player has a Medikit
		if (!("Medikit" in (items player)) ) then
		{
		//systemChat "No revive kit, aborting";
			_return = false;
		};
	}
	else
	{
	//systemChat "Target is either dragged or not unconcious";
	};
	//systemChat "FINAL";
	_return
};
...
FAR_Check_Stabilize = 
{
	...
	// Make sure target is unconscious and hasn't been stabilized yet, and player has a FAK/Medikit
	 // && ( ("FirstAidKit" in (items player)) || ("Medikit" in (items player)) )
	if (_isTargetUnconscious == 1 && _isTargetStabilized == 0 && _isDragged == 0) then
	{
		_return = true;
	};
...
FAR_Check_Suicide =
{
    ...
    _newPlayerObject = _this select 0;
    _isNewPlayerUnconscious = _newPlayerObject getVariable ["FAR_isUnconscious",0];
	//systemChat format["Player is uncon: %1", _isPlayerUnconscious];
	//systemChat format["NewPlayerObj is uncon: %1", _isNewPlayerUnconscious];
	if (alive player && _isPlayerUnconscious == 1) then 
	{
	//systemChat "Player is uncons, can suicide!";

	_Execute = _newPlayerObject getVariable ["FAR_Execute", 0];
	    if (_Execute == 1) then
	    {
	    systemChat "YOU HAVE BEEN EXECUTED";
	    _newPlayerObject allowDamage true;
	    _newPlayerObject setDamage 1;
	    };
		_return = true;
	}else
	{
	//systemChat "Player is cons, cannot suicide!";
	};
